...
elseif($page == 'step')
{
	...
	elseif($step == 1)
	{
		if(isset($_REQUEST['server_path']))
		{
			...
			$config['server'] = $tmp_lua_config->getConfig();
			if(isset($config['server']['sql_type']))
			{
				echo 'File <b>config.lua</b> loaded from <font color="red"><i>'.$path.'config.lua</i></font>. It looks like fine server config file. Now you can check database('.$config['server']['sql_type'].') connection: <a href="install.php?page=step&step=2">STEP 2 - check database connection</a>';
			}
			else
			{
	...
	elseif($step == 3)
	{
		...
		$columns[] = array('guilds', 'description', 'VARCHAR(255)', '', '');
		...
		$columns[] = array('players', 'comment', 'VARCHAR(255)', '', '');
		...
		$columns[] = array('players', 'world_id', 'INT', '11', '0');
	...
	elseif($step == 4)
	{
		...
		$newPlayer = new Player('Player', Player::LOADTYPE_NAME);
		if($newPlayer->isLoaded())
		{
			foreach($samplePlayers as $vocationID => $name)
			{
				...
				{
					$samplePlayer = new Player('Player', Player::LOADTYPE_NAME);
			...
			new Error_Critic('', 'Character <i>Player</i> does not exist. Cannot install sample characters!');
	}
	elseif($step == 5)
	{
