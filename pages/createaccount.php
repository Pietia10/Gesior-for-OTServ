...
function EmailStateChanged() 
{
	...
	$main_content .= 'To play on '.htmlspecialchars($config['server']['servername']).' you need an account. 
						...
						<TR><TD BGCOLOR="'.$config['site']['darkborder'].'"><TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0>
						  <TR><TD>
	...
	if($config['site']['verify_code'])
		$main_content .= '<script type="text/javascript">var verifya=1;</script><TR><TD width="150"><B>Code: </B></TD><TD colspan="2"><img src="pages/imagebuilder.php" border="0" alt="Image Verification is missing, please contact the administrator"></TD></TR>
						  <TR><TD width="190" valign="top"><B>Verification Code: </B></TD><TD colspan="2"><INPUT id="verify" NAME="reg_code" VALUE="" SIZE=30 MAXLENGTH=50><BR><font size="1" face="verdana,arial,helvetica">(Here write verification code from picture)</font></TD></TR>';
	else
	...
	$main_content .= '</TEXTAREA>
					  ...
					  <TR><TD>
					    <TABLE BORDER=0 CELLSPACING=5 CELLPADDING=0>
					    <TR><TD>
					      <INPUT TYPE="checkbox" NAME="rules" id="rules" value="true" /><label for="rules"><u> I agree to the '.htmlspecialchars($config['server']['servername']).' Rules.Go to rules page to read.</u></lable><BR>
					    </TD></TR>
					    <TR><TD>
...
if($action == "saveaccount")
{
	...
	if(empty($reg_form_errors))
	{
		...
		{
			$reg_account->set("premend", $config['site']['newaccount_premdays']);
