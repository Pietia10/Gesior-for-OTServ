...
$config['dotpay_active_transfer'] = true; #active dotpay bank transfers [type=C1] system?
# przykladowy konfig dla SMS
$config['dotpay'][0]['id'] = 23817;       # numer ID zarejestrowanego klienta
$config['dotpay'][0]['code'] = "ALB"; # identyfikator uslug SMS
...
$config['dotpay'][0]['sms_number'] = 76068; # numer na jaki nalezy wyslac kod
$config['dotpay'][0]['sms_text'] = "AP.ALB"; # tresc jaka ma byc w SMSie
$config['dotpay'][0]['sms_cost'] = "7.38 zł brutto"; # cena za wyslanie sms

$config['dotpay'][1]['id'] = 23817;       # numer ID zarejestrowanego klienta
$config['dotpay'][1]['code'] = "ALB2"; # identyfikator uslug SMS
...
$config['dotpay'][1]['sms_number'] = 92578; # numer na jaki nalezy wyslac kod
$config['dotpay'][1]['sms_text'] = "AP.ALB2"; # tresc jaka ma byc w SMSie
$config['dotpay'][1]['sms_cost'] = "30.75 zł brutto"; # cena za wyslanie sms
...
*/
$config['dotpay'][3]['id'] = 77554;       # numer ID zarejestrowanego klienta
$config['dotpay'][3]['code'] = "xmb6"; # identyfikator uslug SMS
$config['dotpay'][3]['type'] = "C1";   # typ konta: C1 - 8 znakowy kod bezobslugowy wysylany mailem, sms - dla sprawdzania SMSow
$config['dotpay'][3]['addpoints'] = 250; # ile premium punktow daje dany sms
$config['dotpay'][3]['sms_number'] = 92058; # numer na jaki nalezy wyslac kod
$config['dotpay'][3]['sms_text'] = "AP.ALBPRZELEW10"; # tresc jaka ma byc w SMSie
$config['dotpay'][3]['sms_cost'] = "10 zł brutto"; # cena za wyslanie sms



$config['dotpay'][4]['id'] = 77554;       # numer ID zarejestrowanego klienta
$config['dotpay'][4]['code'] = "mby8"; # identyfikator uslug SMS
$config['dotpay'][4]['type'] = "C1";   # typ konta: C1 - 8 znakowy kod bezobslugowy wysylany mailem, sms - dla sprawdzania SMSow
$config['dotpay'][4]['addpoints'] = 500; # ile premium punktow daje dany sms
$config['dotpay'][4]['sms_number'] = 92058; # numer na jaki nalezy wyslac kod
$config['dotpay'][4]['sms_text'] = "AP.ALBPRZELEW20"; # tresc jaka ma byc w SMSie
$config['dotpay'][4]['sms_cost'] = "20 zł brutto"; # cena za wyslanie sms
...
if($_REQUEST['system'] == 'dotpay' && $config['dotpay_active'])
{
		...
		if($config['dotpay_active_transfer'])
	{
		...
		foreach($config['dotpay'] as $przelew)
			if($przelew['type'] == 'C1')
				$main_content .= '<br /><b>* Adres - <a href="https://ssl.dotpay.pl/?id='.$przelew['id'].'&code='.$przelew['code'].'"><font color="red">https://ssl.dotpay.pl/?id='.$przelew['id'].'&code='.$przelew['code'].'</font></a> - koszt <font color="red"><b>'.$przelew['sms_cost'].'</b></font>, a za kod dostaniesz <font color="red"><b>'.$przelew['addpoints'].'</b></font> punktow premium.</b>';
	...
	{
		$main_content .= '<h2>SMS</h2><span style="font-size:16px">Kup punkty premium, możesz je wymienić w sklepie OTSa na przedmioty w grze, aby zakupić punkty premium:</span><br />';
		foreach($config['dotpay'] as $sms)
			if($sms['type'] == 'sms')
				$main_content .= '<br /><span style="font-size:15px"><b>* Wyślij SMSa na numer <font color="darkred">'.$sms['sms_number'].'</font> o treści <font color="darkred"><b>'.$sms['sms_text'].'</b></font> za <font color="darkred"><b>'.$sms['sms_cost'].'</b></font>, a za kod dostaniesz <font color="darkred"><b>'.$sms['addpoints'].'</b></font> punktów premium.</b></span>';
		...
		Regulamin: <a href="http://www.dotpay.pl/regulamin-serwisow-sms-premium/" target="_blank">http://www.dotpay.pl/regulamin-serwisow-sms-premium/</a><br />
		Reklamacje: <a href="http://www.dotpay.pl/reklamacje/" target="_blank">http://www.dotpay.pl/reklamacje/</a><br />
		Właścicielem serwisu jest <b>jwalaszczyk10@gmail.com</b> - w razie problemów z płatnością proszę o kontakt na e-mail<br />
		Usługa jest dostępna w sieciach: <b> T-mobile, Orange, Plus GSM, Play, Heyah, Sami Swoi</b>.<br />
...
else
{
	if($config['dotpay_active'])
		$main_content .= '<br /><br /><div style="padding:20px 20px 20px 20px"><center><a href="?subtopic=buypoints&system=dotpay"><h2>For Polish players - Dotpay.pl</h2><h3>Po co przepłacać? Kup punkty w promocyjnej cenie specjalnie dla polaków!</h3><h2>KLIKNIJ TU</h2></a></center></div>';
	...
	if($config['paypal_active'])
		$main_content .= '<br /><br /><div style="padding:20px 20px 20px 20px"><center><a href="?subtopic=paypal"><h2>PayPal</h2><h3>Cheapest points! Send us money from your PayPal account or credit card.</h3><h2>PRESS HERE!</h2></a></center></div>';
	if($config['zaypay_active'])
		$main_content .= '<br /><br /><div style="padding:20px 20px 20px 20px"><center><a href="?subtopic=buypoints&system=zaypay"><h2>ZayPay</h2><h3>Send us money using SMS or phone call.</h3><h2>PRESS HERE!</h2></a></center></div>';
